...
import { renderLogMd, type LogView } from "$src/pages/log.md"
...
describe("renderLogMd with an out-of-range offset", () => {
	it("does not claim an empty log when days exist", () => {
		const document = renderLogMd(outOfRange)
	...
	it("prints an absolute first-page URL to recover in one fetch", () => {
		expect(renderLogMd(outOfRange)).toContain(`${SITE}/log.md\n`)
	})

	it("steps Previous back into the log, not through empty pages", () => {
		expect(renderLogMd(outOfRange)).toContain(`Previous page: ${SITE}/log.md\n`)
	})
})
