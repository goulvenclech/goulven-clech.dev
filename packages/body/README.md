...
- The only runtime state is an append-only log in IndexedDB ([src/logStore.ts](./src/logStore.ts)), exportable and importable as JSON from the Log screen. Entries reference exercises by slug, never sessions, so history survives programme redesigns.
