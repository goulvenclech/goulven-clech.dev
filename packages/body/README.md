...
- The only runtime state is an append-only log in IndexedDB ([src/logStore.ts](./src/logStore.ts)). Entries reference exercises by slug, never sessions, so history survives programme redesigns. Corrections append too: a retraction entry withdraws an earlier one by id, and every reader applies them ([src/corrections.ts](./src/corrections.ts)).
