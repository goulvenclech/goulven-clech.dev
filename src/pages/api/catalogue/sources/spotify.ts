...
async function getToken(): Promise<string> {
  ...
  tokenCache = { value: access_token, expires: Date.now() + (expires_in - 30) * 1000 }
  return access_token
}
...
export function albumCoverUrl(album: SpotifyAlbum, size: "small" | "medium" | "large" = "medium") {
...
export function buildAlbumMeta(album: SpotifyAlbum): string {
  ...
  if (album.artists?.length) parts.push(album.artists.map((a) => a.name).join(", "))
