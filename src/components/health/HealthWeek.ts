...
export class HealthWeek extends HTMLElement {
	...
	private render() {
		...
		const stepValues = daysWithData
			.map((d) => d.steps)
			.filter((v): v is number => v != null)
		const sleepValues = daysWithData
			.map((d) => d.sleep)
			.filter((v): v is number => v != null)
		const energyValues = daysWithData
			.map((d) => d.energy)
			.filter((v): v is number => v != null)
		...
		const averageSleep = sleepValues.length
			? +(totalSleep / sleepValues.length).toFixed(1)
			: 0
		const averageEnergy = energyValues.length
			? Math.round(totalEnergy / energyValues.length)
			: 0
		...
		averagesElement.className =
			"flex aspect-square w-1/8 flex-col items-center justify-center text-xs leading-tight sm:text-sm"
