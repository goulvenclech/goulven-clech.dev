...
Firstly, a <LinkInline to="https://en.wikipedia.org/wiki/Static_web_page">"Static Website"</LinkInline> consists of web pages delivered to the browser exactly as they are stored, unlike a "dynamic" site where a server will generate the pages for every user requests.
...
<FigImage
  ...
  caption="Once your static site is shipped (in blue), every pages will be cached on multiple CDN servers around the world (in pink). When a user requests a page (in green), it will be delivered from the closest CDN server, ensuring good performances and cost efficiency."
/>
...
And even for us, the experience can be frustrating. Without making a complete list, the majority of the SSGs available fall into one of these three faults:
...
<FigImage
  ...
  caption="Somehow, the JS ecosystem is still lacking. Like for CMS, where you have to choose between a hundred of options, but none really usable or decently priced..."
/>
