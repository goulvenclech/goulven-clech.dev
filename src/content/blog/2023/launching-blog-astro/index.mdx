...
Astro won't burden the rocket with a huge JS application, it only generate simple HTML pages. So no client-side JS by default, reducing page weight and ensuring good performance.
...
<FigImage
  ...
  alt="Astro lightouse average score 98, Gatsby 68, Next.JS 63, Wordpress 58"
  caption="An excellent example is Images where, by default, Astro optimizes your image, uses a modern format, includes the loading and decoding attributes, and pushes you to fill the alt attribute (for accessibility) and dimensions (to avoid Cumulative Layout Shift)."
/>
